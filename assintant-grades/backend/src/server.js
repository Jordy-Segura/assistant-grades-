...
async function findUsuarioId(transaction, email) {
  ...
  const result = await new sql.Request(transaction)
    ...
    .query(
      "SELECT TOP 1 IdUsuario FROM aux.UsuariosSistema WHERE Correo = @Correo AND Estado = 1;",
    );
  return result.recordset[0]?.IdUsuario ?? null;
}

async function getOrCreateRAC(transaction, rac, courseConfig) {
  const codigoCarrera = dbString(
    courseConfig.codigoCarreraExterno || courseConfig.carrera,
    "SIN-CARRERA",
  );
  ...
  const result = await new sql.Request(transaction)
    ...
    .input("Descripcion", sql.NVarChar(1000), descripcion).query(`
...
async function getOrCreateRAAU(transaction, raau, courseConfig, idRAC) {
  const codigoAsignatura = dbString(
    courseConfig.codigoAsignaturaExterno || courseConfig.asignatura,
    null,
  );
...
async function getComponenteId(transaction, codigo) {
  const result = await new sql.Request(transaction)
    ...
    .query(
      "SELECT TOP 1 IdComponente FROM aux.ComponentesEvaluacion WHERE Codigo = @Codigo AND Estado = 1;",
    );
  if (!result.recordset[0])
    throw new Error(`No existe el componente ${codigo}`);
  return result.recordset[0].IdComponente;
}

async function getOrCreateProcedimiento(
  transaction,
  idComponente,
  procedimientoCodigo,
  procedimientoNombre,
  ...
  const result = await new sql.Request(transaction)
    ...
    .input("Codigo", sql.NVarChar(30), codigo)
    .input("Nombre", sql.NVarChar(180), nombre).query(`
...
app.get("/api/health", async (_req, res) => {
  ...
  const result = await pool
    .request()
    .query(
      "SELECT 1 AS ok, DB_NAME() AS databaseName, SYSDATETIME() AS serverTime;",
    );
...
app.post("/api/configuraciones", async (req, res) => {
  ...
  try {
    ...
    const configResult = await new sql.Request(transaction)
      ...
      .input(
        "CodigoPeriodoExterno",
        sql.NVarChar(50),
        dbString(
          courseConfig.codigoPeriodoExterno || courseConfig.periodoAcademico,
          "SIN-PERIODO",
        ),
      )
      .input(
        "PeriodoDescripcion",
        sql.NVarChar(180),
        dbString(courseConfig.periodoAcademico, null),
      )
      .input(
        "CodigoFacultadExterno",
        sql.NVarChar(50),
        dbString(
          courseConfig.codigoFacultadExterno || courseConfig.facultad,
          null,
        ),
      )
      .input(
        "FacultadNombre",
        sql.NVarChar(250),
        dbString(courseConfig.facultad, null),
      )
      .input(
        "CodigoCarreraExterno",
        sql.NVarChar(50),
        dbString(
          courseConfig.codigoCarreraExterno || courseConfig.carrera,
          null,
      ...
      .input(
        "CarreraNombre",
        sql.NVarChar(250),
        dbString(courseConfig.carrera, null),
      )
      .input(
        "CodigoAsignaturaExterno",
        sql.NVarChar(50),
        dbString(
          courseConfig.codigoAsignaturaExterno || courseConfig.asignatura,
          null,
      ...
      .input(
        "AsignaturaNombre",
        sql.NVarChar(250),
        dbString(courseConfig.asignatura, "Sin asignatura"),
      )
      .input("PAO", sql.NVarChar(20), dbString(courseConfig.pao, null))
      .input(
        "Aporte",
        sql.NVarChar(50),
        dbString(courseConfig.aporte, "FIN DE CICLO"),
      )
      .input(
        "CodigoDocenteExterno",
        sql.NVarChar(80),
        dbString(
          courseConfig.codigoDocenteExterno || courseConfig.docente,
          null,
        ),
      )
      .input(
        "DocenteNombre",
        sql.NVarChar(250),
        dbString(courseConfig.docente, null),
      )
      .input("CreadoPorUsuarioId", sql.Int, usuarioId).query(`
    ...
    for (const rac of selectedRACs) {
      ...
      await new sql.Request(transaction)
        ...
        .input("IdRAC", sql.Int, idRAC).query(`
    ...
    for (const raau of raauEntries) {
      ...
      if (!idRAC)
        throw new Error(
          `RAAU ${raau.code || raau.codigo} no tiene RAC asociado.`,
        );
      const idRAAU = await getOrCreateRAAU(
        transaction,
        raau,
        courseConfig,
        idRAC,
      ...
      await new sql.Request(transaction)
        .input("IdConfiguracion", sql.Int, idConfiguracion)
        .input("IdRAAU", sql.Int, idRAAU).query(`
    ...
    for (const activity of activities) {
      ...
      const idRAAU =
        raauIdMap.get(activity.raauId) || [...raauIdMap.values()][0];
      if (!idRAAU)
        throw new Error(`Actividad ${activity.name} no tiene RAAU asociado.`);
      ...
      await new sql.Request(transaction)
        ...
        .input(
          "FrontendActivityId",
          sql.NVarChar(80),
          dbString(activity.id, null),
        )
        .input(
          "Nombre",
          sql.NVarChar(180),
          dbString(activity.name, "Actividad"),
        )
        .input(
          "PuntajeMaximo",
          sql.Decimal(5, 2),
          Number(activity.maxScore || 0),
        )
        .input("Orden", sql.Int, order++).query(`
...
app.post(
  "/api/configuraciones/:idConfiguracion/estudiantes",
  async (req, res) => {
    ...
    try {
      ...
      for (const student of students) {
        await new sql.Request(transaction)
          .input("IdConfiguracion", sql.Int, idConfiguracion)
          .input(
            "CodigoEstudianteExterno",
            sql.NVarChar(80),
            dbString(student.id || student.cedula, "SIN-CODIGO"),
          )
          .input(
            "Cedula",
            sql.NVarChar(20),
            dbString(student.cedula || student.id, null),
          )
          .input(
            "Apellidos",
            sql.NVarChar(150),
            dbString(student.apellidos, ""),
          )
...
app.post(
  "/api/configuraciones/:idConfiguracion/calificaciones",
  async (req, res) => {
    const idConfiguracion = Number(req.params.idConfiguracion);
      ...
      for (const grade of grades) {
        ...
        await new sql.Request(transaction)
          ...
          .input(
            "CodigoEstudianteExterno",
            sql.NVarChar(80),
            dbString(grade.studentId, "SIN-CODIGO"),
          )
          .input("Nota", sql.Decimal(5, 2), Number(grade.score)).query(`
...
app.listen(PORT, () => {
  console.log(
    `API Auxiliar de Calificaciones ejecutándose en http://localhost:${PORT}/api`,
  );
});
