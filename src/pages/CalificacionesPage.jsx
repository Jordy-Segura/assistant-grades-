...
export default function CalificacionesPage() {
  ...
  const { activeConfigId, courseConfig, activities } = state;
  const [localGrades, setLocalGrades] = useState({});
  const [saving, setSaving] = useState(false);
  const { load, save, apiAvailable } = useCalificacionesApi(activeConfigId);

  const students = state.students || [];
  const grades = state.grades || [];

  // Load from API on mount / config change
  useEffect(() => {
    if (!activeConfigId) return;
    load().then(data => {
      if (!data) return;
      // Only update if API returned data and we have no local changes
      if (data.grades?.length || data.students?.length) {
        dispatch({ type: "SET_STUDENTS", payload: data.students });
        dispatch({ type: "SET_GRADES", payload: data.grades });
      }
    }).catch(() => {});
  }, [activeConfigId, load, dispatch]);
  ...
  function setGrade(sid, aid, score) {
    setLocalGrades(prev => ({ ...prev, [`${sid}_${aid}`]: score }));
  }
  ...
  function commitLocal() {
    ...
    Object.entries(localGrades).forEach(([key, score]) => {
      ...
      const idx = updated.findIndex(g => g.studentId === sid && g.activityId === aid);
      if (idx >= 0) updated[idx] = { studentId: sid, activityId: aid, score };
      else updated.push({ studentId: sid, activityId: aid, score });
    });
    dispatch({ type: "SET_GRADES", payload: updated });
    setLocalGrades({});
    return updated;
  }

  async function persistGrades() {
    const finalGrades = commitLocal();
    setSaving(true);
    const ok = await save({
      courseConfig,
      activities,
    ...
    setSaving(false);
    window.showToast?.(ok ? "Calificaciones guardadas." : "Error al guardar.", ok ? "success" : "error");
  }
  ...
  const grouped = useMemo(() => COMPONENTS.map(comp => ({ comp, acts: activities.filter(a => a.component === comp) })), [activities]);
  ...
  if (!activeConfigId) {
    return (
      <>
        <div className="page-header"><div className="page-title">Registro de Calificaciones</div><div className="page-sub">Seleccione un PAO para gestionar calificaciones.</div></div>
  ...
  if (students.length === 0) {
    return (
      <><div className="page-header"><div className="page-title">Registro de Calificaciones</div><div className="page-sub">—</div></div>
  ...
  return (
    <>
      <div className="page-header">
        ...
        <div className="page-sub">{courseConfig.asignatura || "—"} · {students.length} estudiantes · {activities.length} actividades{apiAvailable === false ? " · 💾 Local" : apiAvailable ? " · ☁️ BD" : ""}</div>
      </div>
      <div className="comp-bar" style={{ display: "flex", gap: 12, marginBottom: 12, flexWrap: "wrap" }}>
        {grouped.map(g => (
          <div key={g.comp} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: ".75rem" }}>
            ...
            <span style={{ color: "var(--gray-600)" }}>{g.comp}</span>
              ...
              <tr>
                ...
                {grouped.map(g => g.acts.map(a => (
                  <th key={a.id} style={{ position: "sticky", top: 0, background: `${COMPONENT_COLORS[g.comp]}10`, zIndex: 2, minWidth: 60, fontSize: ".7rem", fontWeight: 600, color: COMPONENT_COLORS[g.comp], textAlign: "center" }}>
                    {a.name}<div style={{ fontWeight: 400, fontSize: ".65rem", color: "var(--gray-400)" }}>/ {a.maxScore}</div>
                  </th>
                )))}
                  ...
                  <tr key={s.id}>
                    ...
                    {grouped.map(g => g.acts.map(a => (
                      <td key={a.id} style={{ textAlign: "center", padding: "2px 4px" }}>
                        <input type="number" step="0.25" min="0" max={a.maxScore || 10}
                          ...
                          onChange={e => setGrade(s.id, a.id, e.target.value === "" ? null : parseFloat(e.target.value))} />
                      </td>
                    )))}
                    <td style={{ textAlign: "center", fontWeight: 700, fontFamily: "var(--mono)", color: passed ? "var(--green)" : "var(--red)" }}>{tot.toFixed(2)}</td>
                    <td style={{ textAlign: "center" }}><span className={`badge ${passed ? "badge-green" : "badge-red"}`} style={{ fontSize: ".68rem" }}>{passed ? "✓ Aprobado" : "✗ Reprobado"}</span></td>
                  </tr>
                );
              })}
            </tbody>
  ...
  function exportGrades() {
    const headerRow = `<tr><th rowspan="2">No.</th><th rowspan="2">Código</th><th rowspan="2">Cédula</th><th rowspan="2">Apellidos</th><th rowspan="2">Nombres</th>${grouped.map(g => g.acts.map(() => `<th rowspan="2" style="font-size:9px">${g.comp}</th>`).join("")).join("")}<th rowspan="2" style="font-size:9px">Nota</th></tr>`;
    const rows = students.map((s, idx) => {
      const tot = studentTotal(s.id);
      return `<tr><td>${idx + 1}</td><td>${esc(s.codigo || "")}</td><td>${esc(formatCed(s.cedula))}</td><td>${esc(s.apellidos)}</td><td>${esc(s.nombres)}</td>${grouped.map(g => g.acts.map(a => { const score = getGrade(s.id, a.id); return `<td style="text-align:center">${score != null ? score.toFixed(2) : "—"}</td>`; }).join("")).join("")}<td style="text-align:center;font-weight:700">${tot.toFixed(2)}</td></tr>`;
    }).join("");
    const w = window.open("", "_blank", "width=1400,height=800");
    if (!w) { window.showToast?.("Permita ventanas emergentes.", "error"); return; }
    w.document.write(`<html><head><title>Calificaciones - ${esc(courseConfig.asignatura || "")}</title><style>body{font-family:Inter,Arial,sans-serif;margin:14px;color:#111;font-size:11px}h1{font-size:16px;margin:0 0 4px}.sub{font-size:12px;color:#666;margin-bottom:10px}table{width:100%;border-collapse:collapse;font-size:10px}th,td{border:1px solid #bbb;padding:4px 5px;text-align:left}th{background:#f0f0f0;font-weight:600}</style></head><body><h1>Registro de Calificaciones</h1><div class="sub">${esc(courseConfig.carrera || "")} · ${esc(courseConfig.asignatura || "")} · ${esc(courseConfig.aporte || "")} · PAO ${courseConfig.pao || ""} · Total: ${students.length} estudiantes</div><table><thead>${headerRow}</thead><tbody>${rows}</tbody></table><p style="margin-top:12px;font-size:10px;color:#999">Total máximo: ${maxTotal.toFixed(1)} pts · Fecha: ${new Date().toLocaleString()}</p></body></html>`);
    w.document.close(); w.focus(); w.print();
  }

  function showQR() {
    const compHeaders = grouped.map(g => g.acts.length ? `<th colspan="${g.acts.length}" style="background:#e8f5e9;font-size:10px">${g.comp}</th>` : "").join("");
    const actHeaders = activities.map(a => `<th style="font-size:9px">${esc(a.name)}<br><span style="font-weight:400;color:#888">/${a.maxScore}</span></th>`).join("");
    const tbodyRows = students.map((s, idx) => {
      const tot = studentTotal(s.id);
      return `<tr><td>${idx + 1}</td><td>${esc(s.codigo || "")}</td><td>${esc(formatCed(s.cedula))}</td><td>${esc(s.apellidos)}</td><td>${esc(s.nombres)}</td>${activities.map(a => { const score = getGrade(s.id, a.id); return `<td style="text-align:center">${score != null ? score.toFixed(2) : "—"}</td>`; }).join("")}<td style="text-align:center;font-weight:700">${tot.toFixed(2)}</td><td style="text-align:center">${tot >= 7 ? "✓" : "✗"}</td></tr>`;
    }).join("");
    const qrData = `<div style="font-family:Inter,Arial,sans-serif;padding:20px"><h2 style="font-size:18px;margin:0">${esc(courseConfig.asignatura || "")}</h2><div style="font-size:13px;color:#666">${esc(courseConfig.carrera || "")} · PAO ${courseConfig.pao || ""} · ${esc(courseConfig.aporte || "")}</div><div style="font-size:12px;color:#666;margin-bottom:12px">Docente: ${esc(courseConfig.docente || "")} · ${esc(courseConfig.periodoAcademico || "")}</div><table style="width:100%;border-collapse:collapse;font-size:11px"><thead><tr><th rowspan="2">No.</th><th rowspan="2">Cod.</th><th rowspan="2">Cédula</th><th rowspan="2">Apellidos</th><th rowspan="2">Nombres</th>${compHeaders}${actHeaders}<th rowspan="2">Nota</th><th rowspan="2">Est.</th></tr><tr>${actHeaders}</tr></thead><tbody>${tbodyRows}</tbody></table><p style="margin-top:8px;font-size:11px;color:#999">${new Date().toLocaleString()}</p></div>`;
    const w = window.open("", "_blank", "width=1400,height=800");
    if (!w) { window.showToast?.("Permita ventanas emergentes.", "error"); return; }
    w.document.write(`<html><head><title>QR - ${esc(courseConfig.asignatura || "")}</title><style>body{margin:0}table{border-collapse:collapse}th,td{border:1px solid #ccc;padding:3px 4px;text-align:left}th{background:#f0f0f0}</style></head><body>${qrData}</body></html>`);
